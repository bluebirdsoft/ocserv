...
Download_ocserv(){
	...
	[[ ! -s "ocserv-1.1.6.tar.xz" ]] && echo -e "${Error} ocserv 源码文件下载失败 !" && rm -rf "ocserv/" && rm -rf "ocserv-1.1.6.tar.xz" && exit 1
...
Add_iptables(){
	...
	iptables -t nat -I POSTROUTING 1 -s 192.168.1.0/24 -o enp1s0 -j MASQUERADE
	iptables -I INPUT 1 -i ocs_vpn+  -j ACCEPT
	iptables -I FORWARD 1 -i enp1s0 -o ocs_vpn+ -j ACCEPT
	iptables -I FORWARD 1 -i ocs_vpn+ -o enp1s0 -j ACCEPT
}
Del_iptables(){
...
Set_iptables(){
	...
	sysctl -p
	Network_card=`ip route | grep default | sed -e "s/^.*dev.//" -e "s/.proto.*//"`
	echo  " 检测到网卡为 $Network_card"
